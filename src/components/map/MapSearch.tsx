...
export function MapSearch({
  ...
  const handleInputFocus = (type: 'search' | 'start' | 'end') => {
    ...
    const query =
      type === 'search' ? searchQuery : type === 'start' ? routeQueryStart : routeQueryEnd;
          ...
          <div className="flex flex-col gap-0 bg-background/95 backdrop-blur-md rounded-2xl border border-border/50 shadow-xl animate-in slide-in-from-top-2 overflow-hidden">
            ...
            <div className="flex items-center justify-between px-3 py-2 border-b border-border/50">
              ...
              <Button
                variant="ghost"
                size="icon"
                className="h-6 w-6 rounded-full -mr-1 hover:bg-muted"
                onClick={() => onToggleRouteMode(false)}
              >
                <X className="w-4 h-4" />
              </Button>
            </div>
