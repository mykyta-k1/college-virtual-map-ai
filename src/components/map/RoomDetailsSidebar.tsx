...
export function RoomDetailsSidebar({ room, isOpen, onClose }: RoomDetailsSidebarProps) {
    ...
    const roomPanoramaConfig = hasPanorama && room.panoramaSceneId ? {
        ...panoramaTourConfig,
        default: {
    ...
    return (
        <Sheet modal={false} open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <SheetContent side="right" className="w-full sm:max-w-md p-0 gap-0 border-l border-border/50 shadow-2xl bg-background overflow-hidden flex flex-col">
                {Content}
            </SheetContent>
        </Sheet>
    );
