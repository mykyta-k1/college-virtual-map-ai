...
export function useMapViewerState() {
  ...
  const selectRoom = useCallback(
    (room: SearchableRoom) => dispatch({ type: 'SELECT_ROOM', payload: room }),
    [],
  );
  ...
  const setStartPoint = useCallback(
    (room: SearchableRoom) => dispatch({ type: 'SET_START_POINT', payload: room }),
    [],
  );

  const setEndPoint = useCallback(
    (room: SearchableRoom) => dispatch({ type: 'SET_END_POINT', payload: room }),
    [],
  );

  const setRouteQueryStart = useCallback(
    (query: string) => dispatch({ type: 'SET_ROUTE_QUERY_START', payload: query }),
    [],
  );

  const setRouteQueryEnd = useCallback(
    (query: string) => dispatch({ type: 'SET_ROUTE_QUERY_END', payload: query }),
    [],
  );

  const swapRoute = useCallback(() => dispatch({ type: 'SWAP_ROUTE' }), []);

  const setActiveRouteInput = useCallback(
    (input: 'start' | 'end') => dispatch({ type: 'SET_ACTIVE_ROUTE_INPUT', payload: input }),
    [],
  );

  const setCurrentPath = useCallback(
    (path: PathNode[] | null) => dispatch({ type: 'SET_CURRENT_PATH', payload: path }),
    [],
  );
